...
var scripts = {
	_isJobInList: function (keyVar, argVar, operator) {
		...
		'local function split(inputstr, sep)',
						...
						'return t',
		'end',
		['local splitKey = split(KEYS[1] ,":")'].join(''),
		(keyVar ? 'local items = redis.call("LRANGE", splitKey[1]..":"..splitKey[2]..":active" , 0, -1)' : 'local items = redis.call("LRANGE", KEYS[1], 0, -1)'),
		argVar ? [ operator, ' item_in_list(items, ', argVar, ')' ].join('') : [ operator, ' item_in_list(items, ARGV[1] )' ].join('')
		].join('\n');
	},
	...
	takeLock: function (queue, job, renew, ensureActive) {
		...
		if (ensureActive) {
			...
			var lockAcquired = 'and redis.call("HSET", splitKey[1]..":"..splitKey[2]..":"..' + argVar + ', "lockAcquired", "1")';
