  ...
  _isJobInList: function(keyVar, argVar, operator) {
    ...
    return [
      ...
      ['local splitKey = split(', keyVar, ' , ":")'].join(''),
			'local items = redis.call("LRANGE", table.concat({splitKey[1], splitKey[2], "active"}, ":") , 0, -1)',
  ...
  takeLock: function(queue, job, renew, ensureActive){
    ...
    if (ensureActive) {
      ...
      var isJobInList = this._isJobInList(keyVar, argVar, 'if');
			var lockAcquired = 'and redis.call("HSET", table.concat({splitKey[1], splitKey[2], "active"}, ":"), "lockAcquired", "1")';
      ...
      var opts = {
        lockScript: function(lockScript) {
          return [
			...
			console.log(opts.lockScript('return redis.call("set", KEYS[1], ARGV[1], "NX", "PX", ARGV[2])'));
